...
export async function handleFeedLogic(body) {
  ...
  try {
    ...
    try {
      await addMessageToSession(shop.id, senderId, {
        role: "assistant",
        content: aiResponse.private,
      });
    } catch (e) {
      console.error("History save error (Bot):", e);
    }

    // 5. საჯარო პასუხის ლოგიკა (The Gatekeeper 🛡️)
    let publicAnswer;

    if (shop.answer_publicly) {
      // ✅ თუ ჩართულია: ვიყენებთ AI-ს დაგენერირებულ მოკლე პასუხს
      publicAnswer = aiResponse.private;
    } else {
      // ⛔ თუ გამორთულია: ვიყენებთ პატრონის სტატიკურ ტექსტს
      // Gemini ვერაფერს შეცვლის აქ!
      publicAnswer =
        shop.custom_public_text ||
        "გამარჯობა, დეტალებს პირად შეტყობინებაში მოგწერთ. 😊";
    }
