...
export async function aiResponse(shop, session, userText) {
  ...
  // 1. ჯერ ვამოწმებთ, ხომ არ არის ეს ახალი ძებნა?
  const extractedTerm = await getSearchTerm(userText);

  if (extractedTerm !== "none") {
    // თუ იუზერმა ახალი რაღაც ახსენა (მაგ: "ადიდასი გაქვთ?"), ვეძებთ ახალს
    searchTerm = extractedTerm;
    products = await searchInventory(searchTerm);
  }

  // 2. თუ ახალი არაფერი უხსენებია, მაგრამ დრაფტში უკვე გვიზის პროდუქტი
  if (products.length === 0 && session.draft?.product) {
    console.log(
      "No new product mentioned, keeping context of:",
      session.draft.product,
    );
    searchTerm = session.draft.product;
    products = await searchInventory(searchTerm);
  }
...
export async function parseProductFromText(rawText) {
  ...
  try {
    ...
    if (parsedData && parsedData?.at(0).stock) {
      parsedData.at(0).stock = normalizeStock(parsedData.at(0).stock);
    }
    return parsedData;
