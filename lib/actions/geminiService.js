...
export async function aiResponse(shop, session, userText) {
 const updatedShop = await checkAndGetQuota(shop);


 const limits = { demo: 100, starter: 1000, pro: 5000 };
 const currentLimit = limits[updatedShop.shop_plan] || 100;

 
 if (updatedShop.messages_sent_this_month >= currentLimit) {
   return {
     reply: "ამ მაღაზიას მიმდინარე თვის ლიმიტი ამოეწურა. 🛑",
     extracted: null,
   };
 }
 ...
 let prompt;
  ...
  try {
    const result = await model.generateContent(prompt);
    await incrementMessagesSpent(updatedShop.id);
